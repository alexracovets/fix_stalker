...
interface ElementTableDetailsProps {
  items: NonNullable<ElementsPage['suits_group']>['details']
...
export const ElementTableDetails = ({ items, variant }: ElementTableDetailsProps) => {
    ...
    <AtomWrapper variant="details_list" asChild>
      <ul>
        {items?.map((item, idx) => {
          ...
          const isReserv = reservId.includes((item.indicator as DetaileTable)?.id)
